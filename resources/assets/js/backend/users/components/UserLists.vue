    ...
    <div class="component-wrap">
        ...
        <v-card class="pt-3">
            <v-layout row wrap>
                <v-flex xs12 sm4 class="px-2">
                    <v-btn @click="$router.push({name:'users.create'})" class="blue lighten-1">
                        New User
                        <v-icon right>add</v-icon>
                    </v-btn>
                </v-flex>
                <v-flex xs12 sm8 class="px-2 text-xs-center text-sm-right">
                    <v-btn @click="$router.push({name:'users.groups.list'})" class="blue lighten-1">
                        Manage Groups <v-icon right>group</v-icon>
                    </v-btn>
                    <v-btn @click="$router.push({name:'users.permissions.list'})" class="blue lighten-1">
                        Manage Permissions <v-icon right>vpn_key</v-icon>
                    </v-btn>
                </v-flex>
                ...
                <v-flex xs12 sm4 class="px-2">
                    <v-text-field prepend-icon="search" label="Filter By Email" v-model="filters.email"></v-text-field>
                </v-flex>
                <v-flex xs12 sm4 class="px-2">
                    <v-select box
                              multiple
        ...
        <v-data-table
            ...
            <template slot="items" slot-scope="props">
                ...
                <td>
                    <v-btn small @click="showDialog('user_permissions',props.item.permissions)" outline round color="grey">Show</v-btn>
                </td>
                <td>
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-chip v-for="(permission,key) in dialogs.showPermissions.items" :key="key" class="white--text" :class="{'green':(permission.value==1),'red':(permission.value==-1),'blue':(permission.value==0)}">
                        <v-avatar v-if="permission.value==-1" class="red" title="Deny">
                            <v-icon>block</v-icon>
                        </v-avatar>
                        <v-avatar v-if="permission.value==1" class="green" title="Allow">
                            <v-icon>check_circle</v-icon>
                        </v-avatar>
                        <v-avatar v-if="permission.value==0" class="blue" title="Inherit">
